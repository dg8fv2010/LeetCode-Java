...
public class FrogJump {
    public boolean canCross(int[] stones) {
        ...
        dp.get(0).add(0);
        for (int stone : stones) {
            HashSet<Integer> set = dp.get(stone);
            HashSet<Integer> tmp = new HashSet<>(set);
            for (int j : tmp) {
                for (int delta = -1; delta <= 1; delta++) {
                    int next = stone + j + delta;
                    if (dp.containsKey(next)) {
                        dp.get(next).add(j + delta);
                    }
                }
            }
        }
    ...
    private static void testcase1() {
        FrogJump solu = new FrogJump();
        System.out.println(solu.canCross(new int[]{0, 1, 3, 5, 6, 8, 12, 17}));
        System.out.println(solu.canCross(new int[]{0, 1, 2, 3, 4, 8, 9, 11}));
    }
}
